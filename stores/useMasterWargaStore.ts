...
export const useMasterWargaStore = defineStore('master-warga', {
  ...
  actions: {
    ...
    async fetchAddWarga(params: AddWargaPayload) {
      ...
      try {
        return await api.addWarga(params)
      }
      finally {
        this.loading = false
      }
    ...
    async fetchImportAddWarga(file: File) {
      ...
      try {
        return await api.importAddDataWarga(file)
      }
      finally {
        this.loading = false
      }
    ...
    async fetchUpdateStatus(params: { nik: string; status_keaktifan: 'aktif' | 'tidak_aktif' }) {
      ...
      try {
        return await api.updateStatusWarga(params)
      }
      finally {
        this.loading = false
      }
    ...
    async fetchUpdateWarga(params: AddWargaPayload, nik: string) {
      ...
      try {
        return await api.updateWarga(params, nik)
      }
      finally {
        this.loading = false
      }
    ...
    async fetchDeleteWarga(nik: string) {
      ...
      try {
        return await api.deleteWarga(nik)
      }
      finally {
        this.loading = false
      }
