...
const rules = {
  ...
  jenis_iuran: (v: string) => {
    if (!v)
      return 'Jenis Iuran wajib dipilih'
    if (!['bulanan', 'kematian'].includes(v))
      return 'Jenis iuran tidak valid'
  ...
  periode: (v: string) => {
    if (!v)
      return 'Periode wajib diisi'
    if (!/^\d{4}$/.test(v))
      return 'Periode harus berupa tahun, 4 digit'
    if (Number(v) < 2000)
      return 'Periode minimal tahun 2000'
  ...
  jumlah_iuran: (v: string) => {
    if (!v)
      return 'Jumlah Iuran wajib diisi'
  ...
  nama_warga_meninggal: (v: string) => {
    if (!v)
      return 'Nama Warga yang Meninggal wajib diisi'
  ...
  nik_penanggung_jawab: (v: string) => {
    if (!v)
      return 'Warga Penanggung Jawab wajib diisi'
...
<template>
  ...
  >
    <VCard position="relative">
      <VCardTitle
        class="pt-3 position-fixed top-0 left-0 w-100"
        ...
        >
          ...
          >
            ...
            <template v-if="params.jenis_iuran">
              ...
              <VCol
                v-if="params.jenis_iuran === 'bulanan'"
                cols="12"
              >
                <VSelect
              ...
              <VCol
                v-if="params.jenis_iuran === 'kematian'"
                cols="12"
              >
                <VTextField
